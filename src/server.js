 ...
 * Proxies OpenAI format requests to Anthropic format handler
 */
app.post("/v1/chat/completions", async (req, res) => {
  try {
    ...
    try {
      anthropicRequest = convertOpenAIToAnthropic(openaiRequest);
    } catch (convertError) {
      logger.error("[API] OpenAI conversion error:", convertError);
      return res.status(400).json({
        error: {
          message: "Invalid request format: " + convertError.message,
    ...
    // --- Common Logic (Similar to /v1/messages) ---
    ...
    logger.info(
      `[API] OpenAI Proxy Request for model: ${modelId}, stream: ${!!anthropicRequest.stream}`
    );
    ...
    if (anthropicRequest.stream) {
      ...
      try {
        ...
        for await (const event of sendMessageStream(
          ...
          // Convert Anthropic event to OpenAI chunk
          // Only process relevant events for OpenAI consumers
          if (
            ...
            const openaiChunk = convertAnthropicToOpenAI(event, openaiRequest);
            if (openaiChunk) {
            ...
            const { errorType, errorMessage } = parseError(event.error);
            res.write(
              `data: ${JSON.stringify({
                error: {
    ...
    // Handle auth refresh logic (same as v1/messages)
