...
const VALID_MODEL_PREFIXES = ['claude', 'gemini'];
...
function validateMessage(msg, index) {
    ...
    if (!msg.role || !['user', 'assistant'].includes(msg.role)) {
        return { valid: false, error: `messages[${index}].role must be 'user' or 'assistant'` };
    }
    ...
    if (typeof msg.content !== 'string' && !Array.isArray(msg.content)) {
        return { valid: false, error: `messages[${index}].content must be a string or array` };
    }
    ...
    if (Array.isArray(msg.content)) {
        for (let i = 0; i < msg.content.length; i++) {
            ...
            if (!block || typeof block !== 'object') {
                return { valid: false, error: `messages[${index}].content[${i}] must be an object` };
            }
            if (!block.type) {
                return { valid: false, error: `messages[${index}].content[${i}].type is required` };
            }
        }
    }
...
function validateModel(model) {
    ...
    const hasValidPrefix = VALID_MODEL_PREFIXES.some(prefix => lower.includes(prefix));

    if (!hasValidPrefix) {
        return {
            valid: false,
            error: `Unknown model: ${model}. Model name should contain 'claude' or 'gemini'.`
        };
    }
...
export function validateMessagesRequest(body) {
    ...
    if (body.thinking !== undefined) {
        if (typeof body.thinking !== 'object') {
            ...
            if (typeof body.thinking.budget_tokens !== 'number' || body.thinking.budget_tokens < 1) {
