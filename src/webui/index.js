...
async function setAccountEnabled(email, enabled) {
	const { accounts, settings, activeIndex } =
		await loadAccounts(ACCOUNT_CONFIG_PATH);
...
async function addAccount(accountData) {
	...
	if (existingIndex !== -1) {
		...
		accounts[existingIndex] = {
			...
			invalidReason: null,
			addedAt:
				accounts[existingIndex].addedAt || new Date().toISOString(),
		};
...
function createAuthMiddleware() {
	return (req, res, next) => {
		...
		const isProtected =
			(isApiRoute && !isAuthUrl && !isConfigGet) ||
...
export function mountWebUI(app, dirname, accountManager) {
	...
	app.post("/api/accounts/import", async (req, res) => {
		try {
			...
			for (const acc of importAccounts) {
				try {
					...
					if (!refreshToken && !apiKey) {
						results.failed.push({
							email: acc.email,
							reason: "Missing refresh_token or api_key",
						});
						continue;
					}
			...
			logger.info(
				`[WebUI] Import complete: ${results.added.length} added, ${results.updated.length} updated, ${results.failed.length} failed`,
			);
	...
	app.post("/api/config", (req, res) => {
		try {
			...
			if (
				logLevel &&
				["info", "warn", "error", "debug"].includes(logLevel)
			) {
				updates.logLevel = logLevel;
			}
			if (
				thinkingMode &&
				["default", "inline", "strip"].includes(thinkingMode)
			) {
				updates.thinkingMode = thinkingMode;
			}
			if (
			...
			if (
				typeof rateLimitDedupWindowMs === "number" &&
				rateLimitDedupWindowMs >= 1000 &&
				rateLimitDedupWindowMs <= 30000
			) {
			...
			if (
				typeof extendedCooldownMs === "number" &&
				extendedCooldownMs >= 10000 &&
				extendedCooldownMs <= 300000
			) {
				updates.extendedCooldownMs = extendedCooldownMs;
			}
			if (
				typeof maxCapacityRetries === "number" &&
				maxCapacityRetries >= 1 &&
				maxCapacityRetries <= 10
			) {
			...
			if (success) {
				res.json({
					status: "ok",
					message:
						"Configuration saved. Restart server to apply some changes.",
					updates: updates,
	...
	app.post("/api/models/config", (req, res) => {
		try {
			...
			if (success) {
				...
				res.json({
					status: "ok",
					modelConfig: currentMapping[modelId],
				});
			} else {
	...
	app.get("/api/auth/url", async (req, res) => {
		try {
			...
			const { promise: serverPromise, abort: abortServer } =
				startCallbackServer(state, 120000); // 2 min timeout
			...
			serverPromise
				.then(async (code) => {
					try {
						logger.info(
							"[WebUI] Received OAuth callback, completing flow...",
						);
						const accountData = await completeOAuthFlow(
							code,
							verifier,
						);
						...
						logger.error(
							"[WebUI] OAuth flow completion error:",
							err,
						);
					} finally {
				...
				.catch((err) => {
					...
					if (!err.message?.includes("aborted")) {
						logger.error(
							"[WebUI] OAuth callback server error:",
							err,
						);
					}
					pendingOAuthFlows.delete(state);
	...
	app.post("/api/auth/complete", async (req, res) => {
		try {
			...
			const { extractCodeFromInput, completeOAuthFlow } =
			...
			logger.success(
				`[WebUI] Account ${accountData.email} added via manual callback`,
			);
